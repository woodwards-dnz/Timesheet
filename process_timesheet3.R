...
# timesheet ####
daily <- charges %>% 
  arrange(date, period, project, subproject) %>% 
  group_by(date, period, project, subproject) %>% 
  summarise(
  ...
  dplyr::select(year, period, date, project, subproject, code, subcode, dayhours) %>% 
  mutate(
...
%>% 
  dplyr::filter(period > startdate & period
